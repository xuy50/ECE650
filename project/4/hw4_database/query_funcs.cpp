...
void query2(connection *C, string team_color) {
    ...
    //     " "WHERE NAME = '" + team_color + "' )";
    string queryStr = "SELECT TEAM.NAME FROM TEAM, COLOR WHERE COLOR.NAME = '" +
...
void query4(connection *C, string team_state, string team_color) {
    ...
    //     team_color + "'))";
    string queryStr =
        "SELECT UNIFORM_NUM, FIRST_NAME, LAST_NAME FROM PLAYER, TEAM, STATE, "
        "COLOR WHERE STATE.NAME = '" +
        team_state + "' AND COLOR.NAME = '" + team_color +
        "' AND TEAM.STATE_ID = STATE.STATE_ID AND TEAM.COLOR_ID = "
        "COLOR.COLOR_ID AND PLAYER.TEAM_ID = TEAM.TEAM_ID;";
    /* Create a non-transactional object. */
    nontransaction N(*C);
    /* Execute SQL query */
    result R(N.exec(queryStr));
